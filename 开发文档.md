...
| 2026-09-06 | v0.10 | **维护期增量 · 震动 + 消息通知**：① 触感震动——`lib/haptics.ts` 具名模式表（key 8ms / tap 6ms / confirm 14ms / attention、win、hatch 节奏串 / error 120ms），挂点：`store.press`（三键+键盘统一入口）、图标光标移动、照料动作成功（喂食/打针/冲洗/管教）、宠物事件（与 sfx 同词汇表联动）、游戏胜利；Android Chrome 可用，iOS Safari 无 Vibration API 静默降级；② 消息通知——Web Notifications：宠物叫唤/生病/离世且标签页在后台时提醒（`lib/notify.ts`，tag 按事件类型去重，回前台自动清除），PWA 生产环境经 ServiceWorker `showNotification`（`sw.js` 补 notificationclick 聚焦窗口），开发环境退回页面内 Notification；开关在联机面板工具区（off→点击请求权限→on，denied/unsupported 给出指引），权限状态机 `notifyUi`；③ 偏好持久化——`tk-ui-prefs`（haptics/notifyOn）独立 localStorage 键，属设备本地能力（震动硬件/通知权限各机不同），**不进 SaveFile schema**，避免动 core 的 v1 迁移链。验收：vue-tsc + build + 44 单测 + 5 E2E 全绿；Playwright 探针验证（按键/图标/喂食震动模式、通知权限流转、后台叫唤/生病通知、开关关闭静默）通过 |
